...
namespace Cartpole
{
    ...
    /// </summary>
    public class UprightReward : RewardProvider
    {
        ...
        {
            var upright1 = (Mathf.Cos(pole1.localEulerAngles.z * Mathf.Deg2Rad) + 1) / 2;
            var upright2 = (Mathf.Cos(pole2.localEulerAngles.z * Mathf.Deg2Rad) + 1) / 2;
            var centered1 = Tolerance(pole1.position.x, bounds:(0f, 0f), margin: 2f);
            centered1 = (1 + centered1) / 2;
            var centered2 = Tolerance(pole2.position.x, bounds:(0f, 0f), margin: 2f);
            centered1 = (1 + centered1) / 2;
            var smallVelocity1 = Tolerance(hinge1.Velocity, bounds: (0f, 0f), margin: 5);
            smallVelocity1 = (1 + smallVelocity1) / 2;
            var smallVelocity2 = Tolerance(hinge2.Velocity, bounds: (0f, 0f), margin: 5);
            smallVelocity2 = (1 + smallVelocity2) / 2;
            return upright1 * upright2 * centered1 * centered2 * smallVelocity1 * smallVelocity2; // Multiply, so we need to satisfy all three at the same time
        }
